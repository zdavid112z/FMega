...
mat4 FreetypeGlText::getModelMatrix() const{
    ...
    m[0][1] *= scaling_factor;
    m[0][2] *= scaling_factor;
    m[1][0] *= scaling_factor;
    m[1][1] *= scaling_factor;
    m[1][2] *= scaling_factor;
    m[2][0] *= scaling_factor;
    m[2][1] *= scaling_factor;
    m[2][2] *= scaling_factor;
    return m;
}
...
void FreetypeGl::init(){
...
#else
    default_markup = createMarkup("assets\\fonts\\Roboto-Regular.ttf", 32);
#endif
    ...
    projection = glm::ortho(-500.f, 500.f, -500.f, 500.f, -100000.f, 100000.f);
}
...
void FreetypeGl::preRender() const{
    ...
    glUniformMatrix4fv( glGetUniformLocation( text_shader, "view" ), 1, 0, &view[0][0]);
    glUniformMatrix4fv( glGetUniformLocation( text_shader, "projection" ), 1, 0, &projection[0][0]);
...
void FreetypeGl::renderText(const std::string &text){
    ...
    glUniformMatrix4fv( glGetUniformLocation( text_shader, "model" ),1, 0, &iden[0][0]);
...
void FreetypeGl::renderText(const FreetypeGlText& text, bool call_pre_post) const {
    ...
    glUniformMatrix4fv( glGetUniformLocation( text_shader, "model" ), 1, 0, &text.getModelMatrix()[0][0]);
